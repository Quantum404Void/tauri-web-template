    ...
    ? {
        ...
        deserialize(raw: string): S {
          try {
            ...
            if (import.meta.env.DEV) console.info(`[persist] ${storeId}: parse error, reset`)
          }
          // Return empty object: plugin's pick will filter, store uses its defaults for missing fields
